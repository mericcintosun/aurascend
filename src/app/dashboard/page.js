        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {auraResults.map((aura) => (
              <div
                key={aura.id}
                className="bg-white/5 backdrop-blur-sm rounded-xl overflow-hidden border border-white/10 hover:border-white/20 transition"
              >
                <div
                  className="h-3"
                  style={{
                    backgroundColor: aura.color?.startsWith("from-")
                      ? "#6366f1"
                      : aura.color,
                  }}
                ></div>
